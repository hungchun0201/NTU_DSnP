...
void TaskInitCmd::usage(ostream &os) const
{
   os << "Usage: TASKInit <(size_t numMachines)>" << endl;
}

void TaskInitCmd::help() const
{
...
void TaskRemoveCmd::usage(ostream &os) const
{
   os << "Usage: TASKRemove <-Random (size_t numMachines) |"
      << " -Name (string name)>" << endl;
}

void TaskRemoveCmd::help() const
{
...
TaskQueryCmd::exec(const string &option)
{
   ...
   else if (isValidVarName(token))
   {
      ...
      if (taskMgr->query(n))
      {
         cout << "Query succeeds: " << n << endl;
      }
      else
         cerr << "Query fails!" << endl;
   }
   else
      return CmdExec::errorOption(CMD_OPT_ILLEGAL, token);
   return CMD_EXEC_DONE;
...
void TaskQueryCmd::usage(ostream &os) const
{
   os << "Usage: TASKQuery <(string name) | -HAsh | -HEap | -MINimum >"
      << endl;
}

void TaskQueryCmd::help() const
{
...
TaskAssignCmd::exec(const string &option)
{
   ...
   for (int i = 0; i < repeats; ++i)
   {
      if (taskMgr->assign(load))
         ...
         cerr << "Task assignment fails!" << endl;
   }
   return CMD_EXEC_DONE;
}

void TaskAssignCmd::usage(ostream &os) const
{
   os << "Usage: TASKAssign <(size_t load)> [-Repeat (size_t repeats)]\n";
}

void TaskAssignCmd::help() const
{
