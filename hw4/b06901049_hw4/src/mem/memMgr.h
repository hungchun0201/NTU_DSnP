...
#endif // MEM_DEBUG \
       // TODO
    MemBlock<T> *_searchingBlock = _activeBlock;
    while (_searchingBlock->_nextBlock != 0)
    {

      MemBlock<T> *_searchingBlock2 = _searchingBlock->_nextBlock;
      _searchingBlock->reset();
      delete _searchingBlock;
      _searchingBlock = _searchingBlock2;
    }
...
private:
  ...
  MemRecycleList<T> *getMemRecycleList(size_t n)
  {
    size_t m = n % R_SIZE;
    {
      MemRecycleList<T> *_searchingRecycleList = _recycleList + m;
      if (_searchingRecycleList->_arrSize == n)
      return _searchingRecycleList;
      while (_searchingRecycleList->_nextList != 0)
      {
         MemRecycleList<T> *_searchingRecycleList2 = _searchingRecycleList -> getNextList();
        if (_searchingRecycleList2->_arrSize == n)
        {
          return _searchingRecycleList2;
        }
        _searchingRecycleList = _searchingRecycleList->_nextList;
      }
      MemRecycleList<T> *GeneratedRecycleList = new MemRecycleList<T>(n);
      _searchingRecycleList->_nextList = GeneratedRecycleList;
      return GeneratedRecycleList;
    }
  }
  ...
  T *getMem(size_t t)
  {
    ...
    else
    {
      ...
      {
        size_t retn = _activeBlock->getRemainSize();
        size_t rn = 0;
        if (retn >= S)
        {
          rn = getArraySize(retn);
          getMemRecycleList(getArraySize(retn))->pushFront(ret);
        }

#ifdef MEM_DEBUG
        cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
#endif // MEM_DEBUG
        MemBlock<T> *_cur_active = _activeBlock;
        _activeBlock = new MemBlock<T>(_cur_active, _blockSize);
...
// 6. At the end, print out the acquired memory address
  ...
  size_t getNumBlocks() const
  {
    ...
    size_t counter = 0;
    MemBlock<T> *_searchingBlock = _activeBlock;
    while (_searchingBlock != 0)
    {
      counter++;
      //cerr << _searchingBlock << endl;
      _searchingBlock = _searchingBlock->_nextBlock;
    }
    return counter;
  }
};
