...
void AdtResetCmd::usage(ostream &os) const
{
      os << "Usage: ADTReset <(size_t strLen)>" << endl;
}

void AdtResetCmd::help() const
{
      cout << setw(15) << left << "ADTReset: "
           << "(ADT test) reset ADT\n";
}
...
void AdtAddCmd::usage(ostream &os) const
{
      os << "Usage: ADTAdd <-String (string str) | -Random (size_t repeats)>\n";
}

void AdtAddCmd::help() const
{
      cout << setw(15) << left << "ADTAdd: "
           << "(ADT test) add objects\n";
}
...
AdtDeleteCmd::exec(const string &option)
{
      ...
      else
      {
            ...
            if (myStrNCmp("-Front", options[0], 2) == 0)
                  delType = DO_FRONT;
            else if (myStrNCmp("-Back", options[0], 2) == 0)
                  delType = DO_BACK;
            else if (myStrNCmp("-Random", options[0], 2) == 0)
                  delType = DO_RANDOM;
            else
            ...
            {
            case DO_FRONT:
                  adtTest.deleteFront(iOpt);
                  break;
            case DO_BACK:
                  adtTest.deleteBack(iOpt);
                  break;
            case DO_RANDOM:
                  adtTest.deleteRandom(iOpt);
                  break;
            }
      }
      return CMD_EXEC_DONE;
...
void AdtDeleteCmd::usage(ostream &os) const
{
      os << "ADTDelete < -All | -String (string str) |" << endl
         << "          <<-Front | -Back | -Random> (size_t repeats) >>\n";
}

void AdtDeleteCmd::help() const
{
      cout << setw(15) << left << "ADTDelete: "
           << "(ADT test) delete objects\n";
}
...
void AdtQueryCmd::usage(ostream &os) const
{
      os << "Usage: ADTQuery <(stirng str)>" << endl;
}

void AdtQueryCmd::help() const
{
...
AdtSortCmd::exec(const string &option)
{
      ...
      //  Array<int> _testobj;
      //  cout<<"HI"<<endl;
      //  _testobj.push_back(100);
      //  cout<<"HI2"<<endl;
      //  _testobj.push_back(200);
      //  _testobj.push_back(300);
      //  _testobj.pop_front();
      //  Array<int>::iterator temp = _testobj.begin();
      //  cout<<*temp<<endl;
      //  cout<<*++temp<<endl;
      //  cout<< *temp++ << endl;
      // return CMD_EXEC_DONE;
...
void AdtSortCmd::usage(ostream &os) const
{
      os << "Usage: ADTSort" << endl;
}

void AdtSortCmd::help() const
{
      cout << setw(15) << left << "ADTSort: "
           << "(ADT test) sort ADT\n";
}
...
void AdtPrintCmd::usage(ostream &os) const
{
      os << "Usage: ADTPrint [-Reversed | (int index)]" << endl;
}

void AdtPrintCmd::help() const
{
      cout << setw(15) << left << "ADTPrint: "
           << "(ADT test) print ADT\n";
}
