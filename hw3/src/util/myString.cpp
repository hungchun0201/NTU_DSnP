...
//    presents.
//
//判斷目標字串及輸入字串前n個是否符合。若符合return 0。
int
myStrNCmp(const string& s1, const string& s2, unsigned n)
{
   ...
   for (unsigned i = 0; i < n1; ++i) {
      if (i == n2)
         return (i < n)? 1 : 0;//if the input is too short,return 1;if the length of input is sufiicent,and the first n word is also matched,return 0.
      char ch1 = (isupper(s1[i]))? tolower(s1[i]) : s1[i];
      char ch2 = (isupper(s2[i]))? tolower(s2[i]) : s2[i];//make all contents int string to lower case
      if (ch1 != ch2)
         return (ch1 - ch2);//if input is not matched,return the difference in terms of ASCII code
   }
   return (n1 - n2);
}
