...
HelpCmd::exec(const string& option)
{
   ...
   if (token.size()) {
      ...
      if (!e) return CmdExec::errorOption(CMD_OPT_ILLEGAL, token);//maybe the contents of token is not reged
      e->usage(cout);
   }
   else
      cmdMgr->printHelps();
...
HistoryCmd::exec(const string& option)
{
   ...
   if (!CmdExec::lexSingleOption(option, token))
      return CMD_EXEC_ERROR;
   int nPrint = -1;
   if (token.size()) {//if we assign the wanted number that it should print out,store it in nPrint.if nPrint = -1,print all.
