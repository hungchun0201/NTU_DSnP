...
bool CmdParser::openDofile(const string &dof)
{
  ...
  if (!_dofile->is_open())
  {
    delete _dofile;//without this,the case of overflow may occur
    _dofileStack.empty()?_dofile = 0: _dofile = _dofileStack.top();
    return false;
  }
  _dofileStack.push(_dofile);//store the pointer _dofile into _dofileStack
  return true;
}
...
bool CmdParser::regCmd(const string &cmd, unsigned nCmp, CmdExec *e)
{
  ...
  //進來的e是一個記憶體位置，指向CmdExec。但是在reg時都是new一個derived class的寫法(相當於創了一個object，new出的是其記憶體位置)
  // ，所以實際上e要去實做的function就是去設置該對應object的_optcmd。目前e存到的就是該物件位置
  assert(e != 0);
  e->setOptCmd(optCmd);
...
CmdParser::parseCmd(string &option)
{
  ...
  // TODO...
  string cmd;
  size_t the_end = myStrGetTok(str, cmd);
  CmdExec *e = getCmd(cmd);

  if (the_end == string::npos)
  {
    option.resize(0);
  }
  else
  {
    option = str.substr(the_end, string::npos);
  }
  if (e == 0)
  {
...
{
  CmdExec *e = 0;
  // TODO... finish in 10/21 15:33
  bool is_matched = false;
  //cout << "the input cmd : " << cmd << endl;
  for (int i = 0; i < (int)cmd.size(); i++)
    cmd[i] = toupper(cmd[i]);
  for (CmdMap::iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++)
  {
    // cout << cmd.substr(0, iter->first.size()) << endl;
    // cout << iter->first << " = " << iter->first.size() << endl;
    if (cmd.size() < iter->first.size())
      continue;
    else
    {

      if (cmd.substr(0, iter->first.size()) == iter->first)
      {
        //cout << "this is current cmd : " << cmd << endl;
        if (cmd.substr(0, iter->first.size()) == cmd)
        {
          is_matched = true;
          return iter->second;
        }
        string tesitng_opt = iter->second->getOptCmd();
        string mandatory_part = cmd.substr(0, iter->first.size());
        string optional_part = cmd.substr(iter->first.size());
        // cout << "wanted : " << tesitng_opt << endl;
        // cout << "Mand : " << mandatory_part << endl;
        // cout << "optional : " << optional_part << endl;
        for (int i = 0; i < (int)tesitng_opt.size(); i++)
          tesitng_opt[i] = toupper(tesitng_opt[i]);
        if (optional_part.size() > tesitng_opt.size())
        {
          is_matched = false;
          e = 0;
          break;
        }
        for (int i = 0; i < (int)optional_part.size(); i++)
        {
          // cout << "testing : " << tesitng_opt[i] << endl;
          // cout << "input : " << optional_part[i] << endl;
          if (tesitng_opt[i] == optional_part[i])
          {
            is_matched = true;
            e = iter->second;
          }
...
bool CmdExec::lexSingleOption(const string &option, string &token, bool optional) const
{
  ...
  if (!optional)//mandotary
  {
    if (token.size() == 0)//nothing behind command
    {
      errorOption(CMD_OPT_MISSING, "");
      return false;
    }
  }
  if (n != string::npos)//after getting the token,there is still other string behind it
  {
...
bool CmdExec::lexOptions(const string &option, vector<string> &tokens, size_t nOpts) const
{
  ...
  while (token.size())
  {
    ...
    n = myStrGetTok(option, token, n);//cut the string from pos of previous n
  }
  if (nOpts != 0)
  {
